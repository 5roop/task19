...
nlp = classla.Pipeline('hr',processors='tokenize,ner,pos,lemma,depparse',use_gpu=False)
...
# for file in os.listdir('/home/rupnik/parlamint/ParlaMint/Data/ParlaMint-HR/'):
for file in ["ParlaMint-HR_T06_S12.xml"]:
    if not file.endswith('.xml') or 'classla' in file:
        continue
    file='/home/rupnik/parlamint/ParlaMint/Data/ParlaMint-HR/'+file
    print(file)
    tree = ET.parse(file)
    root = tree.getroot()
    i=0
    ...
    for seg in tqdm(root.iter('{http://www.tei-c.org/ns/1.0}seg')):
        i+=1
        seg_seq=[]
        if seg.text!=None:
